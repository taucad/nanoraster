...
export const verifyPreviewInstall = ({ from, metadata, sha, install = execFileSync }) => {
  ...
  try {
    ...
    // `npm install` honours os/cpu/libc, so the root install only lands the
    // packages this runner can use — two of seventeen here. `npm pack` fetches
    // a manifest regardless of platform, which is the only way to see what the
    // other fifteen actually published.
