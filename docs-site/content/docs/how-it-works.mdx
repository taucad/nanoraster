...
rasterises and encodes once per view. A profiled call's
[`RenderProfile`](api#renderprofile) fields map one-to-one onto these stages —
`parseMs` for parse and fit, `setupMs` for device acquisition and upload, then
per-view `renderMs` and `encodeMs` — so the stage narrative above doubles as
the profile-field reference.
