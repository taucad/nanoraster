...
geometry, primitive visibility, and any section caps before moving geometry to
...
and [Cut section views](guides/cut-section-views).

**Rasterise** clips and draws surfaces, section caps, cap outlines, and authored
lines, then applies the requested annotations. Surface lighting uses the
studio preset or supplied rig; section caps are unlit evidence. Failures here
are GPU-class.
...
fields map onto these stages: `parse` for parse and fit, `capBuild` for shared
presentation work, `upload` for source geometry, then per-view `render`,
`overlay` for the annotation pass, and `encode`. `setup` measures the complete
acquisition, presentation and upload interval.

[Both hosts](install), Node.js native and
[browser WebGPU](guides/render-in-the-browser), wrap the same Rust core and
codecs. Only GPU rasterisation differs, so bytes may differ by a shade (see
[Determinism](#determinism)) but any structural difference between hosts is a
bug.
...
render targets, whose lifetime is the process or worker. A `views` plan is the
work: every requested image in one value, so the core can schedule the next
view's GPU pass while the previous one encodes, which a loop over single
renders can never grant.

A renderer keeps the device, shader, pipelines and the last output size's
render targets between calls; parsing, camera fitting and the geometry upload
happen per call. The one-shot functions run through one shared renderer per
process: created on the first call, run in call order, never disposed, and
trimmed of render targets larger than 2048² after each call so one large
render does not hold that memory.
...
the request carries a [rig](guides/light-the-subject); tone map and gamma are
fixed; fitted placement is derived from referenced indexed geometry; camera
angles, format, size, background and annotations are yours.
...
and compare cross-host perceptually rather than by hash. Any
[lossless format](guides/format-and-annotate) works as a baseline, but a
...
showed. Metals read as grey at small sizes, because only an analytic
environment is there to reflect; prefer a dielectric for thumbnails. Authored
edge lines are drawn in their own pass, unaffected by the material factors.
With `lighting` held constant, a difference between two renders of the same
model is a material difference and nothing else.
