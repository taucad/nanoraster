...
**Rasterise** draws the model under lighting that is fixed in this release, then
draws authored edge lines. Failures here are GPU-class.
...
removes inputs rather than seeding them. Lighting, environment, tone map and
gamma are fixed in this release; camera distance is derived from the model's
bounds; angles, format, size, background and annotations are yours.

What is guaranteed:

- Repeating a request on the same host produces identical bytes, in the same
  process or a different one.
- Lighting is identical across every render.
...
is three numbers and a colour, not an image. That restriction lets a render be
reproduced from the model file alone. A texture-backed material makes the
output depend on decode behaviour, sampler state and mip selection, none of
which the GLB pins down, so two hosts can disagree about the same file.
...
unaffected by the material factors. Because lighting is fixed in this release,
a difference between two renders of the same model is a material difference
and nothing else.
