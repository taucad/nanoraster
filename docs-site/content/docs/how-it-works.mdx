...
flowchart TD
  subgraph CPU1[CPU]
    ...
    B --> C[Resolve fitted or fixed camera]
  end
  subgraph GPU[GPU]
...
**Resolve and upload** either fit referenced indexed geometry from a direction
and margin or preserve a fixed position, target, up vector, projection, and
clipping request, then move the geometry to the device once; see [Frame the model](guides/frame-the-model)
and [Place the camera](guides/place-the-camera).

**Rasterise** draws the model under the studio preset or the rig you supply,
tone maps with the ACES filmic curve, then draws authored edge lines. Failures
here are GPU-class.
...
fields map onto these stages: `parse` for parse and fit, `setup` for device
acquisition and upload, then per-view `render`, `overlay` for the annotation
pass, and `encode`.
