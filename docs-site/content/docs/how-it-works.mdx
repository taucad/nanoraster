...
**Resolve and upload** either fit the model bounds from a direction and margin
or preserve a fixed position, target, up vector, projection, and clipping
request, then move the geometry to the device once; see [Frame the model](guides/frame-the-model)
...
fixed; camera distance is derived from the model's bounds; angles, format,
size, background and annotations are yours.
