...
guess. With `format: 'raw'` this stage is skipped and the frame is returned as
read back; see [Work with raw pixels](guides/work-with-raw-pixels).
...
same pathway through one shared renderer per process. It is created on the
first call, runs calls in order, and is never disposed. After each call it
drops render targets larger than 2048², so one very large render does not
...
Any [lossless format](guides/format-and-annotate) works as a baseline: PNG,
or WebP at the default `quality: 1`, both decode to the same pixels. A
byte-locked baseline also locks the encoder. nanoraster's WebP encoder is a
maintained fork whose lossless bytes can change in a release, so a version
bump resets the baseline whichever format you chose, as a render fix
does. [`format: 'raw'`](guides/work-with-raw-pixels) removes the encoder from
the comparison: a diff over the frames can only report the render.
