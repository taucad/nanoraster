...
input; a failure here is classified `parse`. An out-of-profile _material_ is
...
guess. `format: 'raw'` is this stage doing nothing: the frame comes back as it
was read, which [Work with raw pixels](guides/work-with-raw-pixels) covers.

[`renderImages`](api#renderimages) parses and uploads once, then fits,
rasterises and encodes once per view. A timed call's
[`RenderTimings`](api#rendertimings) fields map one-to-one onto these stages —
`parse` for parse and fit, `setup` for device acquisition and upload, then
per-view `render` and `encode` — so the stage narrative above doubles as
the timings-field reference.
...
The design model is one sentence: **state crosses the boundary as handles;
work crosses as plans.** A [`Renderer`](api#renderer) is the state — the GPU
device, compiled shader, pipelines, and render targets, everything whose
natural lifetime is "the process or worker". A `views` plan is the work — the
whole set of requested images in one value, which is what lets the core
schedule the next view's GPU pass while the previous one is still encoding. A
JavaScript loop over single renders can never grant that, because the core
only ever sees one image at a time.

What a renderer keeps between calls: the device and queue, the shader module,
one pipeline set per stroke width it has seen, and the render targets for the
last output size. What stays per call: parsing, camera fitting, and the
geometry upload — each call brings its own GLB. The one-shot functions are the
same pathway with the lifetime folded in: they share one renderer per process,
created on the first call, serialized in call order, and never disposed. It
drops render targets above 2048² after each call, so one enormous render
cannot pin that memory for the life of the process.

Reuse changes no pixels. Device and target reuse skip re-creation, not
arithmetic, so a warm renderer's output is byte-identical to the one-shot
functions on the same adapter — the conformance suite pins this. If the
device is lost between calls (driver reset, browser reclaim), the renderer
rebuilds it and re-uploads on the next call; the loss surfaces once, as a
GPU-class failure on the call that hit it.
...
Any [lossless format](guides/format-and-annotate) serves as a baseline — PNG,
or WebP at its default `quality: 1` — because both decode to the same pixels.
What a byte-locked baseline also locks is the encoder: nanoraster's WebP
encoder is a maintained fork whose lossless bytes can move in a release, so a
version bump resets the baseline whichever format you chose, exactly as a
render fix does. [`format: 'raw'`](guides/work-with-raw-pixels) is the
comparison with no encoder in it at all: a diff over the frames themselves
can only report the render.
