...
[retry decision](guides/handle-render-failures) is mechanical, not a guess.
With `format: 'raw'` the renderer skips this stage and returns the frame as
...
nanoraster renders glTF 2.0 metallic-roughness materials, embedded PNG/JPEG/WebP
maps and the physical material extensions listed in the compatibility guide.
Image decoding and mip generation run in the shared Rust core on every host.
A fixed model, camera and lighting rig therefore has the same material inputs
on native and browser backends.

`metallicFactor` blends dielectric and metallic responses.
...
the base colour. A metal has no diffuse lobe, so the prefiltered room environment
...
The studio uses a neutral room environment, an integrated GGX lookup table,
multiple-scattering compensation and PBR Neutral tone mapping. Glass samples
retained HDR scene mip levels with bicubic filtering. Unlit materials bypass
lighting but still receive exposure and tone mapping. Authored edge lines
remain in their own pass, unaffected by material factors.
With `lighting` held constant, a difference
