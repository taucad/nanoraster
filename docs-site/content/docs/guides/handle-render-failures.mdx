...
_software_ adapter — SwiftShader, lavapipe, WARP — that renders correctly but
an order of magnitude slower. `describeAdapter` is the preflight that
separates the two before you commit to a workload:

```typescript
import { describeAdapter } from 'nanoraster';

const adapter = await describeAdapter().catch(() => undefined);

if (adapter === undefined) {
  // adapter-unavailable: fall back now, nothing will render.
} else if (adapter.includes('(Cpu)')) {
