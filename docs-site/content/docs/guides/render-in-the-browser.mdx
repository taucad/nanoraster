...
Rendering fails with a typed error when no adapter exists; check first and
choose a fallback before any work.
...
you got without downloading the wasm; `'cpu'` means software rasterisation, an
order of magnitude slower. Handle
