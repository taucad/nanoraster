...
```typescript
export declare const renderDirectionFromOrbit: (orbit: RenderOrbit, world?: RenderWorld) => RenderVector3;
```
...
```typescript
import { renderDirectionFromOrbit } from 'nanoraster';

const world = { up: '+z', forward: '-y' } as const;
const direction = renderDirectionFromOrbit({ azimuth: 45, elevation: 30 }, world);
```

### `renderOrbitFromDirection`

```typescript
export declare const renderOrbitFromDirection: (direction: RenderVector3, world?: RenderWorld) => RenderOrbit;
```
...
- **Returns:** the orbit angles that reproduce it, exactly inverting
  [`renderDirectionFromOrbit`](#renderdirectionfromorbit) in the same world.
