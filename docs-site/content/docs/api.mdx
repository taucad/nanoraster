...
- **Returns:** a newly allocated `render.<format>` file with the width and
...
- **Device loss is recovered** on the next call, which rebuilds the device and
  re-uploads.
- `dispose()` waits for in-flight calls to settle, is idempotent, and backs
  `using` through `Symbol.dispose`; every later call rejects with
  `RenderError` code `'gpu'`, so recreate rather than retry.
...
declares itself a fallback, and reject with `adapter-unavailable` when none is
usable.
...
you forward the plan call through a generic wrapper of your own:
...
override, falling back to the shared format. Entry files are named
`render-<id>.<format>`.
...
acquisition plus geometry upload), then per-view rasterise and encode, all in
milliseconds.
