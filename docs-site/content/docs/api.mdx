...
Defaults are fit direction `[0.6123724357, 0.5, 0.6123724357]`, up `[0, 1, 0]`,
minimum margin `0.1`, and perspective field of view `45` with zoom `1`.
`direction` fixes orientation from the subject toward the camera; fitting may
translate the optical axis to balance the projected geometry. A fixed camera
requires non-coincident position/target and a non-collinear up vector. Omitted
clipping derives safe planes from referenced geometry. Explicit range outside
the referenced subject is tightened without changing visible geometry.
...
| `renderImageMaxSections`              | `6`                                       | planes per request          | `sections.planes`           |
