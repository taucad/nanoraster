...
A fixed camera requires non-coincident position/target. A view collinear with
`up` names no roll, so screen-up becomes `world.forward`, or `world.up` along
forward. Omitted clipping derives safe planes from referenced geometry. Explicit
range outside the referenced subject is tightened without changing visible
geometry.
