...
const image = await renderImage(glb, {
  ...
  lineWidth: 2, // output pixels; default
...
nanoraster solve framing; a fixed camera preserves caller-owned pose and
projection.
...
margin `0.1`, and perspective field of view `45` with zoom `1`. A fixed camera
requires non-coincident position/target and a non-collinear up vector. Omitted
clipping derives safe planes from the bounds.
...
| `renderImageMarginRange`              | `readonly [0, 0.5]`                       | fraction of fitted extent   | `camera.margin`             |
