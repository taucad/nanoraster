...
Save this as `render.mjs` and run it with `node render.mjs`. The fitted camera
direction is written out so you can edit it; omitting `camera` uses the same
three-quarter default. The live tile renders at 960×720, so its byte count differs, and its
milliseconds drop after the first frame because dragging reuses one renderer;
[Reuse the renderer](/docs/guides/reuse-the-renderer) is the same move in your
own code.
