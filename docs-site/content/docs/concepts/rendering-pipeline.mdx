...
browser tab — need to produce the _same_ image from the same model. Meeting that
...
**Rasterise** draws the model under the studio rig, or the one you passed as
`lighting`, tone maps the result with the ACES filmic curve, then draws authored
edge lines. Failures here are GPU-class.
...
**Lighting is a defaulted stage input.** The studio rig applies unless the
request carries a `lighting` block; the ACES tone map and the sRGB transfer
after it are fixed either way. Why the default exists is explained in
[Determinism](determinism); how to replace it is
