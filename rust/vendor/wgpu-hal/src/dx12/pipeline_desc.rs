...
///
/// Each subobject must start on a pointer-aligned boundary. Each subobject contains a 32 bit
...
impl<'a> RenderPipelineStateStream<'a> {
    ...
    fn add_object<T: RenderPipelineStreamObject>(&mut self, object: T) {
        // nanoraster patch (upstream gfx-rs/wgpu: `align_to(8)`): D3D12 parses a
        // pipeline state stream with each subobject aligned to the size of a
        // pointer (d3dx12's `alignas(void*)` subobject wrapper), so on 32-bit
        // Windows the 8-byte padding misaligns every subobject after the first
        // and `CreatePipelineState` fails with E_INVALIDARG on WARP and real
        // drivers alike. Align to the pointer size (8 on 64-bit, unchanged)
        // or the subobject's own alignment, whichever is larger.
        self.align_to(align_of::<*const c_void>().max(align_of_val::<T>(&object)));
