...
The default studio matches Tau's Three.js **0.184.0** neutral-room profile.
...
2. Click **Save lighting assets**. The page reads its live half-float PMREM,
   verifies equal RGB channels, and saves compressed little-endian 16-bit red-channel values.
...
mip -2. The 16-bit red-channel values are rounded to seven mantissa bits (three dropped bits),
then packed losslessly into a 384×1024 RGBA8 PNG. Decoding returns bytes for
a **768×1024 16-bit floating-point red-channel** texture; the PNG channels are byte storage, not colors.
...
`dfg.bin` retains the configured 16×16 two-channel half-float lookup from Three.js 0.184.0. Its axes are
